<template>
    <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center sm:pt-0">
        <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block space-x-4">
            <router-link to="/dashboard" class="text-sm text-gray-700 underline" v-if="this.$store.state.authToken">
                Dashboard
            </router-link>

            <template v-else>
                <router-link to="/login" class="text-sm text-gray-700 underline">
                    Log in
                </router-link>

                <router-link to="/register" class="ml-4 text-sm text-gray-700 underline">
                    Register
                </router-link>
            </template>
        </div>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-center pt-8 sm:justify-start sm:pt-0">
                <BreezeApplicationLogo class="block w-20 h-20 w-auto" />
                <span class="text-7xl font-semibold text-red-500">Lumen</span>
            </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4 text-lg leading-7 font-semibold">
                                <a href="https://lumen.laravel.com/docs" class="underline text-gray-900 dark:text-white">Documentation</a>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        getData() {
            axios.get('/welcome')
                .then((response) => {
                    this.lumenVersion = response.data.lumenVersion;
